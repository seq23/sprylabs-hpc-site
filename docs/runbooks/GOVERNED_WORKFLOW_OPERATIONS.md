...
7. **Validators read rendered pages from the repository root.** The site is served from the repository root; the separate staged public root, and the `run_with_public_root` shim that wrapped validators for it, were removed when that layout was retired. Validators now read the page tree directly.
