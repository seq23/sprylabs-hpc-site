...
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'sprylabs-reopen-')); execFileSync('unzip',['-q',path.resolve(zip),'-d',temp]); const entries=fs.readdirSync(temp); if(entries.length!==1) fail('[validate:reopened-baseline] ZIP must contain one repository wrapper'); const root=path.join(temp,entries[0]); const req=readJson('_baseline_packaging_contract.json').required_files; const errors=req.filter(x=>!fs.existsSync(path.join(root,x))).map(x=>`missing ${x}`);
