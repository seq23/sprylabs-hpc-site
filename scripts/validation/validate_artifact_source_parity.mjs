...
const ZIP_MAX_BUFFER = 64 * 1024 * 1024;
const required=readJson('_baseline_packaging_contract.json').required_files; const entries=execFileSync('unzip',['-Z1',zip],{encoding:'utf8', maxBuffer: ZIP_MAX_BUFFER}).trim().split(/\n/).filter(Boolean); const wrapper=entries[0]?.split('/')[0]; const errors=[]; const hashes={};
for(const rel of required){const entry=`${wrapper}/${rel}`; if(!entries.includes(entry)){errors.push(`ZIP missing ${rel}`);continue;} const source=fs.readFileSync(rel); const zipped=execFileSync('unzip',['-p',zip,entry],{maxBuffer: ZIP_MAX_BUFFER}); const a=crypto.createHash('sha256').update(source).digest('hex'); const b=crypto.createHash('sha256').update(zipped).digest('hex'); hashes[rel]={source:a,artifact:b}; if(a!==b) errors.push(`hash mismatch ${rel}`);}
