...
function walkDeployAssets(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (['.git', '.pages-output', 'node_modules', '.validation-runtime', '.validation-cache', '.build-cache'].includes(entry.name)) continue;
...
function walk(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    if (['.git', '.pages-output', 'node_modules', '.validation-runtime', '.validation-cache', '.build-cache', 'artifacts', 'test-results', 'playwright-report'].includes(entry.name)) continue;
    const full = path.join(dir, entry.name);
