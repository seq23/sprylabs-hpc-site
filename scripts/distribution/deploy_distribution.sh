...
if [[ -n "$GSC_CREDS" && -n "$GSC_SITE_URL" && -f "$GSC_CREDS" ]]; then
  if [[ -n "$HOST" ]]; then
    ...
    python3 scripts/distribution/gsc_submit_sitemaps.py "$GSC_CREDS" "$GSC_SITE_URL" "${sitemaps[@]}" || true
  else
...
if [[ -n "$GSC_CREDS" && -n "$GSC_SITE_URL" && -f "$GSC_CREDS" ]]; then
  if [[ -n "$HOST" ]]; then
    python3 scripts/distribution/gsc_inspect_urls.py "$GSC_CREDS" "$GSC_SITE_URL" "$PRIORITY_FILE" "${ARTIFACT_DIR}/inspection-results.json" || true
  else
