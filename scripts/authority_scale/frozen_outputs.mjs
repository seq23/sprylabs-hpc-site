...
import {SHRINK_BYTES, SHRINK_RATIO, SHRINK_ACCEPTANCES_REL, classifyShrinks, loadShrinkAcceptances, writeShrinkAcceptances} from './frozen_output_shrink.mjs';
...
// The thresholds, and the named acceptances that lower a floor on purpose,
// live in ./frozen_output_shrink.mjs so the pull-request check
// (validate:frozen-output-shrink) and this writer apply one rule.
function auditShrink(old, records) {
  ...
  for (const [route, next] of Object.entries(records)) {
    ...
    shrunk.push({ route, path: next.path, before, after, lost, ratio: Number((lost / before).toFixed(4)), frozen_sha256: prev.sha256 });
  }
  shrunk.sort((a, b) => b.lost - a.lost);
  const { material, accepted } = classifyShrinks(shrunk, loadShrinkAcceptances(ROOT).acceptances);
  if (accepted.length) {
    console.log(JSON.stringify({
      notice: 'FROZEN_OUTPUT_SHRINK_ACCEPTED',
      source: SHRINK_ACCEPTANCES_REL,
      accepted: accepted.map(({ route, before, after, floor_bytes, reason }) => ({ route, before, after, floor_bytes, reason })),
    }, null, 2));
  }
  if (shrunk.length) {
    console.log(JSON.stringify({
  ...
  if (material.length && process.env.FROZEN_OUTPUT_ACCEPT_SHRINK !== '1') {
    console.error(JSON.stringify({
      ...
      message: `${material.length} page(s) would be re-frozen having lost at least ${SHRINK_BYTES} bytes and 10% of their content. Re-accepting these makes the thinner output the new baseline and the loss becomes invisible. Establish that the trim is intended, then record it as a named acceptance in ${SHRINK_ACCEPTANCES_REL} (route, from_sha256, floor_bytes, reason) - or, for a one-off, re-run with FROZEN_OUTPUT_ACCEPT_SHRINK=1.`,
