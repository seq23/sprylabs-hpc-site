...
for(const name of CFG.public_entries){const rootPath=path.join(ROOT,name);const st=fs.lstatSync(rootPath,{throwIfNoEntry:false});if(staged){if(!st&&!compat.has(name))errs.push(`staged public entry missing:${name}`)}else if(st&&!compat.has(name))errs.push(`public entry leaked into committed root:${name}`)}
