...
//
// download.html's BYTES remain frozen at a known hash -- it is the revenue
// surface. What is no longer frozen is its route: the owner authorised one
// narrow edit (five characters, the `.html` removed from the canonical href)
// so the page finally declares the URL that actually serves 200. /download.html
// 301s; /download is the live page. Keeping .html here made the tag, sitemap and
// manifest agree with each other while all three disagreed with the server.
//
// The set stays, empty, because the byte-freeze contract still references it and
// a future frozen route may need routing held. Nothing routes to .html today.
const FROZEN_HTML_ROUTES = new Set();
...
function hostFor(route, publishedHostOverrides = new Map()) {
  ...
  // /amazon/ holds the companion pages for the published books. They belong to
