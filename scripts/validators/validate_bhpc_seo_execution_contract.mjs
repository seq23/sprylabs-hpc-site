...
const report={schema_version:'1.0',generated_at:new Date().toISOString(),status:errors.length?'FAIL':'PASS',normalized_path:latest?`${NORMALIZED_ROOT}/${latest}`:'',declared_count:payload.seo_execution_count||0,validated_count:rows.length,errors,warnings};
...
for(const w of warnings)console.warn(`[validate:bhpc-seo-execution] WARN ${w}`);
console.log(`[validate:bhpc-seo-execution] PASS: validated=${rows.length}; warnings=${warnings.length}; source=${report.normalized_path}`);
