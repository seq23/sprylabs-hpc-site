...
function checkProtected(){ for(const fileName of ['product.html','download.html']){ const p=path.join(ROOT,fileName); if(fs.existsSync(p)){ const html=fs.readFileSync(p,'utf8'); if(/data-auto-patched="true"/.test(html)) fail(fileName+' contains automated patch marker'); } } }
