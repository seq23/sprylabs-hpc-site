...
const first = {
  ...
  source_record_ids: ['record-001', 'record-002'],
  source_entry_ids: ['record-001', 'record-002'],
...
const duplicate = {...first, id: 'record-002', record_id: 'record-002', source_record_ids: ['record-002']};
const distinct = {
  ...base,
  id: 'record-003',
...
const html = renderSemanticEvidence([first, distinct]);
...
const rerendered = renderSemanticEvidence([first, distinct]);
assert.equal(html, rerendered, 'rendering is byte deterministic');

console.log('[bhpc-agent-semantic-evidence:self-test] PASS: duplicate consolidation, source alias proof, distinct record proof, visible strings, and deterministic rendering');
